// Edge AI endpoint using fetch (Node SDK doesn't run in Edge)
export const config = { runtime: "edge" };

function corsHeaders(req) {
  const origin = req.headers.get("origin") || "https://ireacch.github.io"; // pin to your site
...
export default async function handler(req) {
  ...
  const sys = `You are an analyst writing brief, executive-friendly insights for a research-culture survey (Likert 1–5; 4–5 positive, 1–2 negative, 3 neutral). Always: use only provided numbers; start with a 2–3 sentence summary with exact numbers; call out risks/strengths; then 3 short actions. If userPrompt exists, answer in 1–3 sentences using only the data or say it's not answerable.`;
